...
local function handleInput(input)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- For touch input, adjust the position to match screen space
        if input.UserInputType == Enum.UserInputType.Touch then
            local screenPosition = Vector2.new(position.X, position.Y)
            position = UserInputService:GetMouseLocation() - UserInputService:GetMouseDelta()
        end
...
-- Connect the function to both MouseButton1Down and Touch inputs
Mouse.Button1Down:Connect(function()
    handleInput({UserInputType = Enum.UserInputType.MouseButton1, Position = UserInputService:GetMouseLocation()})
end)
...
-- Assuming tab5 is previously defined and set up correctly
tab5:CreateToggle("Tap Delete", OnToggleChanged)
...
local function onInput(input, gameProcessed)
    if not gameProcessed then
        -- Check if the input is from a controller and is the dpad up
        if UserInputService:GetLastInputType() == Enum.InputType.Gamepad and input.KeyCode == Enum.KeyCode.DPadUp then
            movePlayerUp(5)  -- Move the player up by 5 studs
            SpawnBaseplateAtMiddle()  -- Spawn the baseplate at the middle
        end
    end
...
local function movePlayerDown(distance)
    ...
    local humanoidRootPart = character:WaitForChild("HumanoidRootPart")
    local currentPosition = humanoidRootPart.Position
    local newPosition = currentPosition - Vector3.new(0, distance, 0) -- Move down
    humanoidRootPart.CFrame = CFrame.new(newPosition)
end

-- Function to delete baseplate
...
-- Callback function for Dpad press
local function onDpadDownPressed(input)
...
-- Connect input action to callback
UserInputService.InputBegan:Connect(onDpadDownPressed)

-- Assuming `tab5` and `CreateButton` are defined elsewhere in your code
-- Add button to trigger move down and delete baseplate for manual testing
local function Callback()
    movePlayerDown(5)  -- Move the player down by 5 studs
    DeleteBaseplate()  -- Delete the baseplate from the workspace
end
